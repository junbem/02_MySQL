...
SELECT
    menu_code
     ...
     ,   menu_price
FROM
    tbl_menu -- 오름차순이 기본값 -> menu code
ORDER BY
    #         menu_price ASC; -- ASC는 오름차순 (기본값)
#         menu_price; -- (오름차순 기본값)
        menu_price DESC; -- DESC는 내림차순

-- --------------------------------------------------------------------------------------
-- ORDER BY 절을 사용해서 결과집합을 여러 열로 정렬
-- --------------------------------------------------------------------------------------
SELECT
    menu_code
     ,   menu_name
...
-- --------------------------------------------------------------------------------------
-- 일단 가격순으로 내림차순으로 저장하고, 가격이 같으면 메뉴이름의 오름차순으로 정렬함.
-- --------------------------------------------------------------------------------------
...
-- FIELD (검색할문자열, target1, targe2, ...) 검색 문자열과 일치하는 타겟의 인덱스를 반환함
...
SELECT
    menu_name 메뉴,
    FIELD(orderable_status, 'Y', 'N') "주문 가능여부"
FROM
    tbl_menu;
...
/*
📌 FIELD(value, list...) → value가 list에서 몇 번째인지 반환
📌 FIELD()를 ORDER BY에 사용하면 사용자 지정 순서로 정렬 가능
📌 FIELD(orderable_status, 'Y', 'N') → 'Y' 먼저, 'N' 나중
📌 즉, ORDER BY FIELD()를 사용하면 일반적인 알파벳/숫자 정렬이 아닌, 원하는 순서대로 정렬할 수 있다! 🚀
 */
-- --------------------------------------------------------------------------------------

-- NULL
-- 오름차순 시 NULL 값은 맨 위에 온다 (DEFAULT)
-- 내림차순 시 NULL 값은 맨 밑에 온다 (DEFAULT)
SELECT
    category_code
     ,   category_name
     ,   ref_category_code
