...
FROM tbl_menu; -- 버전별로 한글 또는 영문에 대한 BYTE 크기가 다름
...
SELECT
    FIND_IN_SET('바나나', '사과,딸기,바나나'); -- 리스트 칸띄우면 안됨
    -- >> 언제 쓰는가? -> 최상위로 올릴 때 아래와 같이 사용..
SELECT * FROM tbl_menu ORDER BY FIND_IN_SET(menu_code, 15) DESC;
...
SELECT INSTR('사과딸기바나나', '바나나'), LOCATE('딸기', '사과딸기바나나')  ;
...
SELECT FORMAT(123412341234.5121345, 3);
SELECT menu_name, FORMAT(menu_price,0) FROM tbl_menu;
...
-- UPPER: 소문자를 대문자로 변경
-- LOWER: 대문자를 소문자로 변경
SELECT UPPER('hihihi'), LOWER('Hello World');
...
-- SPACE(길이)
-- SPACE: 길이만큼의 공백을 반환
SELECT CONCAT('제 이름은', SPACE(5), '이고 ,나이는', SPACE(3), '세 입니다.') 나이소개;

-- SUBSTRING(문자열, 시작위치, 길이)
-- SUBSTRING: 시작 위치부터 길이만큼의 문자를 반환(길이를 생략하면 시작 위치부터 끝까지 반환)

SELECT
        SUBSTRING('안녕하세요 반갑습니다.', 7, 6) "반가움"
    ,   RPAD(SUBSTRING('881122-1234567', 1, 8), 14, '*') "뒷자리 가림"
    ,   SUBSTRING('안녕하세요 반갑습니다.', 7) "반가움 2";
...
-- ROUND: 반올림값 반환
SELECT  CEILING(124.6) "1"
    ,   FLOOR(125.3) "2"
    ,   TRUNCATE(1234.123456, 3) "3"
    ,   ROUND(124.6) "4"
    ,   ROUND(1234.567) "5"
    ,   TRUNCATE(-0.15, 0) "6";
...
SELECT ADDDATE('2025-03-22', INTERVAL 30 DAY), ADDDATE('2025-03-22', INTERVAL 50 MONTH);
SELECT SUBDATE('2025-03-22', INTERVAL 30 DAY), SUBDATE('2025-03-22', INTERVAL 50 MONTH);
SELECT ADDDATE('2025-03-22', INTERVAL 1 MONTH);
...
-- 년월일은 더하거나 뺄 수 없다.
SELECT ADDTIME('2024-03-22 15:19:00', '3:0:1');
SELECT SUBTIME('2024-03-21 15;19:00', '4:0:0');
...
SELECT NOW(), LOCALTIME(), LOCALTIME, LOCALTIMESTAMP(), LOCALTIMESTAMP, SYSDATE();


-- YEAR(날짜), MONTH(날짜), DAY(날짜),
-- HOUR(시간), MINUTE(시간), SECOND(시간), MICROSECOND(시간)
-- 날짜 또는 시간에서 연, 월, 일, 시, 분, 초, 밀리초를 추출
SELECT  YEAR(CURTIME()) 년, MONTH(CURDATE()) 월, DAY(CURDATE()) 일
    ,   HOUR(NOW()) 시, MINUTE(NOW()) 분, SECOND(NOW()) 초, MICROSECOND(NOW()) 밀리초;
...
SELECT DATEDIFF(NOW(), '1993-03-22'), TIMEDIFF('18:00:00', '15:32:00'), DATEDIFF('2025-09-09', NOW()) "엔코아 탈출";
...
SELECT EXTRACT(DAY_HOUR FROM NOW()); -- HOUR, DAY_HOUR은 동일
SELECT EXTRACT(MINUTE FROM NOW());
SELECT EXTRACT(SECOND FROM NOW());
SELECT EXTRACT(MICROSECOND FROM NOW());
SELECT EXTRACT(MICROSECOND FROM '2025-03-22 15:39:18.113344');
...
SELECT IFNULL(null, 'Hello World'), IFNULL('ㅋㅋㅋㅋㅋ', 'Hello World');
...
SELECT
        menu_name
    ,   IF(orderable_Status = 'Y', '주문가능', '주문 불가' ) orderable_Status
...
SELECT
    ...
    ,   CASE
        ...
        ,   WHEN menu_price < 5000 THEN '싼거'
        ,   WHEN menu_price < 10000 THEN '적당히 싼거'
        ,   WHEN menu_price < 15000 THEN '비싼거'
    , ELSE
