...
-- LENGTH : 할당된 바이트(BYTE) 크기

SELECT menu_name, BIT_LENGTH(menu_name) FROM tbl_menu;
...
SELECT  * FROM  tbl_menu ORDER BY FIND_IN_SET(menu_code,12) DESC, FIND_IN_SET(menu_code, 10) DESC;
...
SELECT FORMAT(123412313123412341234.516554,3);
...
SELECT LEFT('Hello World', 4), RIGHT('Hello world\d', 5);
...
SELECT MOD(75, 10), 75 % 10, 75 MOD 10; -- 다 같은 방법
...
SELECT ADDDATE('2024-03-21 15:19:00', '3:0:1'), SUBDATE('2024-03-21 15:19:00', '4:0:1');
...
-- EXTRACT(UNIT FROM 날짜시간) : 특정한 시간이 필요할 때 뽑아낼 수 있게 도와주는 함수
...
SELECT EXTRACT(MICROSECOND FROM NOW()); -- NOW에는 ms는 없음.
SELECT EXTRACT(MICROSECOND FROM '2025-03-21 15:12:33, 11112222'); -- NOW에는 ms는 없음.
...
SELECT 5 > '6MAY'; -- 바꿀 수 있는데까진 바꿈.
SELECT 5 > '6MAY', '6MAY' + 1; -- 바꿀 수 있는데까진 바꿈.
...
SELECT IFNULL(null, 'Hello World'), IFNULL('ㅋㅋㅋㅋ','Hello World');
SELECT IFNULL(null, 'Hello World'), IFNULL('나는 널 아니다','Hello World');
...
SELECT
        menu_name
    , IF(orderable_status = 'Y', '주문 가능', '주문 불가') orderable_staus
...
-- *는 모든 컬럼(하나의 행)을 의미한다. 행이 존재하면 1로 카운트
SELECT
         COUNT(*) -- NULL 포함 COUNT
