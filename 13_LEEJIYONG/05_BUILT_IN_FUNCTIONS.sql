...
-- FIND_IN_SET: 찾을 문자열의 위치 반환, 문자열 리스트의 구분자는 반드시 ,여야 한다. (띄어쓰기 NO)
...
SELECT
  ...
  FROM
       tbl_menu
 ORDER BY
       FIND_IN_SET(menu_code, 12) DESC, FIND_IN_SET(menu_code, 13) DESC;
...
-- 1,000,000 -> 문자열임
SELECT
       FORMAT(1000000.13221, 0)
     , FORMAT(1000000.12332114, 3);
...
SELECT ROUND(1234.56), ROUND(1234.567, 2), TRUNCATE(1234.56, 1), TRUNCATE(-0.65, 0), ROUND(-0.65, 0);
...
-- 지원하는 날짜/시간 단위
SELECT NOW(), ADDDATE(NOW(), 10), SUBDATE(NOW(), 10);

SELECT ADDDATE('2024-03-21', INTERVAL 30 DAY), ADDDATE('2024-03-21', INTERVAL 5 MONTH);
SELECT SUBDATE('2024-03-21', INTERVAL 30 DAY), SUBDATE('2024-03-21', INTERVAL 5 MONTH);
...
-- 년월일은 더하거나 뺄 수 없다.
SELECT ADDTIME('2024-03-21 15:19:00', '1:1:1'), SUBTIME('2024-03-21 15:19:00', '4:1:1');
...
SELECT EXTRACT(DAY_HOUR FROM NOW()), EXTRACT(HOUR FROM NOW()); -- HOUR, DAY_HOUR는 동일하다.
SELECT EXTRACT(MINUTE FROM NOW());
SELECT EXTRACT(SECOND FROM NOW());
SELECT EXTRACT(MICROSECOND FROM NOW());
SELECT EXTRACT(MICROSECOND FROM '2025-03-21 15:12:33.123456789');
