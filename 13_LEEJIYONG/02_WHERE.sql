...
#       orderable_status = 'n';
orderable_status = 'N';
-- MySQL은 비교나 검색들을 수행할 때 기본적으로 대소문자 구분없이 비교 및 검색이 가능하다.
...
-- 0은 False, 0외 숫자는 TRUE로 암시적으로 형변환 후에 평가된다.
...
-- 메뉴테이블에서 주문여부가 Y이면서, 카테고리 코드가 10인 메뉴목록을 조회
SELECT menu_code
     , menu_price
     , category_code
     , orderable_status
...
-- 메뉴테이블에서 메뉴가격이 5,000원보다 크고, 카테고리 코드가 10인 메뉴를 출력해주세요.
...
-- 메뉴 테이블에서 주문여부가 Y이거나, 카테고리코드가 10인 메뉴목록을 조회
SELECT menu_code
     , menu_price
     , category_code
     , orderable_status
...
-- 우선순위
-- 나열한 AND OR 중에는 AND가 우선순위가 더 높다.
SELECT 1 OR 0 AND 0;
SELECT (1 OR 0) AND 0;


-- 카테고리번호가 4 또는 가격이 9,000원이면서 메뉴번호가 10번보다 큰 메뉴를 조회하자.
-- 모든 컬럼을 조회
SELECT *
FROM tbl_menu
...
-- IFNULL(_ , _) : NULL이면 두번째 매개변수값으로 변환
SELECT
       category_code
     , category_name
     , ref_category_code
     , IFNULL(ref_category_code, '널값입니다.')
  FROM
       tbl_category
 WHERE
...
-- escape문자 : \% -> 99.99\%
-- tb_escape_watch 테이블에서 decription컬럼에 99.99%라는 글자가 들어가 있는 행만 추출하세요.
SELECT
       watchname
     , description
  FROM
