...
# LIKE / NOT LIKE            문자 패턴 비교 - 조건을 뽑아내고 싶을 때
# IS NULL / IS NOT NULL      NULL 여부 비교
# IN / NOT IN                비교 값 목록에 포함/미포함 되는지 여부 비교 - or의 느낌이랑 비슷함
# WHERE 논리 연산자
...
-- tbl_menu 테이블에서 가격이 13000원인 메뉴이름, 메뉴가격 , 주문여부칼럼을 출력
SELECT
          menu_name AS 메뉴이름
        , menu_price AS 메뉴가격
        , orderable_status AS 주문여부칼럼
    FROM
         tbl_menu
    WHERE
        menu_price = 13000;

-- 같지 않을 연산자와 함꼐 where절 사용
SELECT
          menu_name AS 메뉴이름
        , menu_price AS 메뉴가격
        , orderable_status AS 주문여부칼럼
...
#            orderable_status != 'Y';
#            orderable_status = 'N';
           orderable_status = 'n'; -- MySQL은 비교나 검색을 수행할 때 기본적으로 대소문자 구분없이 비교 및 가능하다.
...
-- 0은 FALSE, 0와 숫자는 TRUE롤 암시적 형변환 후 평가된다.
...
-- 카테고리번호가 4 또는 가격이 9000원이면서 메뉴번호가 10번보다 큼 메뉴를 조회하자.
