...
-- CONCAT(문자열1, 문자열2, ...), CONCAT(구분자, 문자열1, 문자열2, ...)
-- CONCAT: 문자열 이어붙임
-- CONCAT_WS: 구분자와 함계 문자열을 이어붙임
...
SELECT  * FROM tbl_menu ORDER BY FIND_IN_SET(menu_code, 12) DESC;
...
SELECT FORMAT(123412341234.123412,3);
...
SELECT
#       INSERT(menu_name, INSTR(menu_name, '쥬스'), 2, 'JUICE') -- 원본은 그대로
      *
FROM
      tbl_menu
 WHERE
...
-- 년월일은 더하거나 뺄 수 없다.
SELECT ADDDATE('2024-03-21 15:19:00', '1:0:1'), SUBDATE('2024-03-21 15:19:00', '4, 0,1');
...
SELECT EXTRACT(MINUTE_MICROSECOND FROM '2025-03-21 15:12:33.11112222');
...
-- 1 SUM
-- 해당 컬럼 값드의 총 합을 구하는 함수
-- 해당 컬럼의 값이 null인 행은 제외하고, 합계를 한다.
SELECT menu_price FROM tbl_menu;
SELECT CONCAT(SUM(menu_price), '원')FROM tbl_menu;
...
-- 내부적으로 함계와 COUNT 정보를 가지고 여산을 한다.
SELECT AVG(menu_price) FROM tbl_menu;
...
SELECT
      COUNT(*) -- NULL 포함 카운트
    , COUNT(ref_category_code) -- NULL 미포함 카운트
FROM tbl_category;
