...
-- tbl_menu 테이블에서 가격이 130000원인 메뉴이름, 메뉴가격, 주문여부 컬럼 출력
SELECT
        menu_name
     ,  menu_price
     ,  orderable_status
...
SELECT 1 AND 1, 2 && 2, -1 && 'abc';
SELECT 1 AND 0, 0 AND 1, 0 AND 0, 0 AND NULL;
SELECT 1 AND NULL, NULL AND NULL;
SELECT 1 + NULL, 1 - NULL; -- NULL과의 연산 값은 NULL
...
-- 메뉴가격이 5000원보다 크고, 카테고리 코드가 10인 메뉴 출력
SELECT
        menu_code
     ,  menu_name
     ,  menu_price
...
FROM
    tbl_menu
WHERE
      menu_price BETWEEN 1000 AND 25000;
...
-- escape문자 : \%
-- tb_escape-watch 테이블에서 description컬럼에 99.99%라는 글자가 들어가있는 행만 추출하세요.
SELECT
      watchname
    , description
  FROM
