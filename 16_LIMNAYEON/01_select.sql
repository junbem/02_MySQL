...
-- --------------------------------------------------
-- FROM절이 없는 SELECT문 (자기가 쓴 것이 컬럼명)
SELECT 7 + 3;
SELECT 10 * 2;
SELECT 6 % 3, 6 % 4;
SELECT NOW(); -- 현재 시간
...
-- --------------------------------------------------
-- 메뉴 이름은 생갈치쉐이크이고, 가격은 30,000원 입니다.
SELECT
...
-- 별칭(alias)
SELECT 7 + 3 AS '합';
SELECT NOW() '현재 시간'; -- 별칭을 달 때, 별칭에 특수기호나 칸 띄우기가 필요하면 싱글쿼테이션을 반드시 입력해야한다.
...
SELECT
      ...
      , '원'
   FROM tbl_menu
  ORDER BY 2 DESC; -- 2행 기준 정렬

-- --------------------------------------------------
-- DISTINCT
-- 컬럼에 포함된 중복 값을 한번씩만 표시하고자 할 때 사용
-- SELECT문에 한 번만 사용 가능하고, 여러 컬럼에 대해서 사용하면,
-- 두 컬럼 값에 대해 중복 값을 제거한다.
