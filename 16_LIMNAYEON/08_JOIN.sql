...
SELECT
        a.menu_name
      , b.category_name
  FROM tbl_menu a -- 베이스 테이블, 왼쪽 기준
  LEFT JOIN tbl_category b USING(category_code); -- 오른쪽 기준
-- ======================== RIGHT JOIN ================================
SELECT
        a.menu_name
      , b.category_name
...
-- CROSS JOIN은 조건 절이 없다. 또는 JOIN문의 ON절이 없을 경우
SELECT
        a.menu_name
      , b.category_name
  FROM  tbl_menu a
...
SELECT
  ...
  LEFT JOIN location d ON d.NATIONAL_CODE = d.NATIONAL_CODE
