...
-- AND &&    여러 조건이 동시에 TRUE일 경우에만 TRUE 값 반환, 앞이 false면 뒤에를 굳이 보지 않음
-- OR        여러 조건들 중에 어느 하나의 조건만 TRUE이면 TRUE값 반환, 앞이 ture면 굳이 보지 않음
...
-- NULL과의 연산 결과는 null이다(0 && null 제외)

SELECT 1 AND 1, 2 && 2, -1 && 1, 1 && 'ABC';

SELECT 1 AND 0, 0 AND 1, 0 && 0, 0 AND NULL;

SELECT 1 AND NULL, NULL AND NULL;

SELECT 1 + NULL, 1 - NULL, 1 * NULL, 1 / NULL;

-- 메뉴 테이블에서 주문 여부가 Y이면서(AND), 카테고리 코드가 10인 메뉴 목록을 조회
SELECT
        menu_code
      , menu_price
      , category_code
...
-- 메뉴 테이블에서 메뉴 가격이 5000원보다 크고, 카데고리 코드가 10인 메뉴를 출력해 주세요.
...
-- 메뉴 테이블에서 주문 여부가 Y이거나(OR), 카테고리 코드가 10인 메뉴 목록을 조회
SELECT
        menu_code
      , menu_price
      , category_code
...
-- 카테고리 번호가 4 또는 가격이 9,000원이면서 메뉴번호가 10번보다 큰 메뉴를 조회하자.
SELECT
        menu_code
        , menu_price
        , category_code
