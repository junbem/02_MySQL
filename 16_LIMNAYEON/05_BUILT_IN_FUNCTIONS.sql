...
SELECT  FIND_IN_SET('바나나', '사과,딸기,바나나'); -- 띄어쓰기 하면 안 됨
...
SELECT
        INSTR('사과딸기바나나', '딸기'), -- 정방향
        LOCATE('딸기', '사과딸기바나나'); -- 역방향 // 문자를 오른쪽에서부터 찾지만 반환되는 인덱스는 왼쪽이 시작이다.

-- FORMAT(숫자, 소수점 자릿수)
-- FORMAT: 1000단위마다 콤마(,) 표시를 해 주며 소수점 아래 자릿수(반올림)까지 표현한다.
-- FORMAT(number, decimal_places)

SELECT FORMAT(123412341234123212.516465, 3);
...
SELECT
        INSERT(menu_name, INSTR(menu_name, '쥬스'), 2, 'JIICE')
  FROM
        tbl_menu
 WHERE
        menu_name LIKE '%쥬스%'
...
SELECT LEFT('Hello World!', 4), RIGHT('Hello Wrold', 5);
...
SELECT
    ...
    DATE_FORMAT(NOW(), '%Y/%m/%d(%W)'); -- %w 일요일(0) ~ 토요일(6) 숫자반환
...
# null 처리함수
-- IFNULL(EXPRESSION, NULL일 때 대체값)
SELECT IFNULL(null, 'Hello World'), IFNULL('ㅎㅎㅎㅎㅎ', 'Hello World');
