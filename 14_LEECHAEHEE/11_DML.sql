...
-- 문법2. NULL 허용 가능한(NULLABLE) 컬럼이나 AUTO_INCERMENT가 있는 컬럼을 제외하고
...
UPDATE
   ...
   SET
      category_code = 10
    , menu_name = '매생이쉐이크' -- 여러 개의 값을 동시에 수정할 경우 콤마(,) 사용
  WHERE
       menu_code = 26;
-- SUBQUERY를 활용할 수도 있다.
-- 다만 MySQL은 Oracle과 달리 update나 delete시 자기 자신 테이블의 데이터를 사용하면 1093 에러가 발생한다.
UPDATE
    tbl_menu
SET
...
-- 이때 SUBQUERY를 하나 더 사용하여 임시테이블로 사용하게 하면 해결할 수 있다.
UPDATE
    tbl_menu
SET
