...
-- 서브쿼리가 다중형을 반환하는 경우 = 연산자를 사용할 수 없음
SELECT * FROM tbl_menu WHERE category_code IN (SELECT category_code FROM tbl_category WHERE ref_category_code = 1);
...
SELECT -- 인라인 뷰에 존재하는 컬럼만 참조할 수 있다.
     v.menu_name
   , v.category_name
   , v.menu_price
