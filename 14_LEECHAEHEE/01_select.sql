...
SELECT NOW();
SELECT CONCAT('유', ' ', '관순');
-- 메뉴 이름은 생참치쉐이크이고, 가격은 30000원 입니다.
SELECT CONCAT('메뉴 이름은 ', menu_name, '이고 ', '가격은 ', menu_price, ' 원입니다.') AS '메뉴 정보'
FROM tbl_menu;

-- 별칭(alias) 달아보기
SELECT 7 + 3 AS '합';
SELECT NOW() AS '현재 시간'; -- 별칭을 달 때 별칭에 특수기호나 칸 띄우기가 필요하면 작은따옴표(single quotation) 사용
...
SELECT
     ...
     , '원'
FROM tbl_menu
ORDER BY 2 DESC; -- 1번, 2번으로 사용할 수 있으나 가능한 컬럼명을 쓰기

-- DISTINCT
-- 컬럼에 포함된 중복 값을 한번씩만 표시하고자 할 때 사용
-- SELECT문에 한번만 사용 간ㅇ하고, 여러 컬럼에 대해서 사용하면 두 컬럼값에 대해 중복을 제거함.
