...
SELECT FORMAT(2123412341234.234, 3);
...
-- 881122-1******
SELECT LPAD('왼쪽', 6, '@'), RPAD('오른쪽', '6', '@'); SELECT RPAD('881122-', 14, '*');
...
SELECT
       DATEDIFF('2025-09-09', NOW()), TIMEDIFF('18:00:00', '15:32:12');
...
SELECT EXTRACT(HOUR FROM NOW()); -- HOUR, DAY_HOUR는 동일
SELECT EXTRACT(MINUTE FROM NOW());
SELECT EXTRACT(SECOND FROM NOW());
SELECT EXTRACT(MICROSECOND FROM '2025-03-21 15:40:32.112332');
...
SELECT CAST(AVG(menu_price) AS SIGNED INTEGER) '평균 가격' FROM tbl_menu;
SELECT CONVERT(AVG(menu_price), SIGNED INTEGER) '평균 가격' FROM tbl_menu;
...
SELECT '1' + '2';
SELECT CONCAT(menu_price, '원') FROM tbl_menu; -- menu.price가 문자로 변환된다.
SELECT 3 > 'MAY'; -- 문자는 0으로 변환
SELECT 5 > '6MAY', '6MAY' + 1;
SELECT ADDDATE('2025-03-11', 1); -- 날짜형으로 바뀔 수 있는 문자는 DATE형으로 변환됨
...
SELECT  menu_name,
        IF(orderable_status = 'Y', '주문 가능', '주문 불가') orderable
   FROM tbl_menu;
...
SELECT menu_name,
     ...
     , CASE
        ...
        WHEN menu_price BETWEEN 5000 AND 10000 THEN '적당히 싼거'
        WHEN menu_price > 10000 THEN '비싼거'
ELSE '많이 비싼거'
END 가격레벨
FROM tbl_menu;
...
-- 해당 컬럼의 값이 null인 행은 제외하고 합계를 구함
...
-- 3. COUNT
-- 테이블에서 조건을 만족하는 행의 개수 반환
-- *는 모든 컬럼(하나의 행)을 의미한다. 행이 존재하면 1로 카운트
SELECT
      COUNT(*) -- NULL 포함 COUNT
    , COUNT(ref_category_code) -- NULL은 세지 않음
FROM
     tbl_category;
