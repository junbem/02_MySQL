...
SELECT
        ...
        ,category_name
    FROM tbl_menu
    JOIN tbl_category tc on tbl_menu.category_code = tc.category_code;
...
-- 메뉴 이름은 참치쉐이크이고, 가격은 30,000 원 입니다.
SELECT CONCAT('메뉴 이름은 ', menu_name, '이고, 가격은 ', menu_price, '원 입니다.') FROM tbl_menu;

-- 별칭 (alias) 달아보기-----------------------------

SELECT  7 + 3 AS 합;
SELECT  NOW() '현재 시간'; -- 별칭을 달 때 별칭에 특수기호나 칸띄우기가 필요하면 ''(싱글쿼테이션)을 사용하면 된다.


-- 리터럴(값)
-- 임의로 지정한 문자열은 SELECT 절에 사용하면, 테이블에 존재하는 데이터처럼 사용할 수 있다.
-- 문자 혹은 날짜 리너털은 ''기호를 사용해야 한다.
-- 리터널은 ResultSet의 모든 행에 반복표시 된다.
SELECT
        menu_name 메뉴이름,
...
-- DISTINCT
-- 컬럼에 포함된 중복 값을 한벅씩만 표시하고자 할 때 사용
-- SELECT문에 한번만 사용가능하고, 여러 칼럼에 대해서 사용하면,
-- 두 칼럼값에 대해 중복값을 제거한다.
