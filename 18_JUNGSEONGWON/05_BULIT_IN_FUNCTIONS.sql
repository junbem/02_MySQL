...
SELECT menu_name,  BIT_LENGTH(menu_name),CHAR_LENGTH(menu_name),LENGTH(menu_name) FROM tbl_menu;-- 단일 행 처리 함수 행마다 처리를 한다.
...
-- FIND_IN_SET: 찾을 문자열의 위치 반환, 문자열 리스트의 구분자는 반드시 ,여야 하며 문자열 간의 띄어쓰기를 해서는 안된다.

SELECT FIND_IN_SET('바나나', '사과,딸기,바나나');

-- ORDER BY에서 특정 레코드를 끌어올리기 위한 용도로 사용할 수 있다.
SELECT  * FROM  tbl_menu ORDER BY FIND_IN_SET(menu_code, 15) DESC, FIND_IN_SET(menu_code,16)DESC ;  -- 맨 위로 12번 코드를 올려주는 것
...
SELECT
    INSTR('사과딸기바나나','사과'),
    LOCATE('바나나','사과딸기바나나'); -- 해당 내용의 문자열을 찾는 순서는 왼쪽에서 오른쪽으로 동일하다.

-- FORMAT(숫자, 소수점 자릿수)
-- FORMAT: 1000단위마다 콤마(,) 표시를 해 주며 소수점 아래 자릿수(반올림)까지 표현한다.
-- FORMAT(number, decimal_places)

SELECT  FORMAT(12341231243132.124345,3);
...
SELECT
    ...
    FROM
         tbl_menu
   WHERE
         menu_name LIKE '%쥬스%'; -- 조회 된 내용에서만 변경이니 전체 메뉴 내용에서는 안 바뀐다.
...
SELECT LOWER('Hello wolrd'), UPPER('Hello world');

-- LPAD(문자열, 길이, 채울 문자열), RPAD(문자열, 길이, 채울 문자열)
-- LPAD: 문자열을 길이만큼 왼쪽으로 늘린 후에 빈 곳을 문자열로 채운다.
-- RPAD: 문자열을 길이만큼 오른쪽으로 늘린 후에 빈 곳을 문자열로 채운다. , 비밀번호 및 주민등록번호 등 중요한 정보를 가릴 때 주로 사용
-- 881122-1******
...
SELECT TRIM('     MYSQL    '), TRIM(BOTH '@' FROM '@@@@MTSQL@@@@');

-- REPEAT(문자열, 횟수)
-- REPEAT: 문자열을 횟수만큼 반복

SELECT REPEAT('재밌어요', 100);
...
SELECT
      ...
      ,SUBSTRING('안녕하세요 반갑습니다.', 7); -- 길이에 대한 내용이 없으면 끝까지 다 나온다.
...
SELECT ADDTIME('2024-03-21 15:19:00', '3:0:1'), SUBTIME('2024-03-21 15:19:00', '4:0:1');
...
SELECT HOUR(CURTIME()), SECOND(CURTIME()), MICROSECOND(CURTIME());
...
SELECT DATEDIFF('2025-2-21', NOW()),
       TIMEDIFF('18:00:00', '15:32:12');
...
SELECT EXTRACT(DAY_HOUR FROM NOW());  -- HOUR와 DAY_HOUR는 동일하다.
SELECT EXTRACT(HOUR FROM NOW());  -- HOUR와 DAY_HOUR는 동일하다.
SELECT EXTRACT(MINUTE FROM NOW());
SELECT EXTRACT(SECOND FROM NOW());
SELECT EXTRACT(MICROSECOND FROM '2025-03-21- 15:12:33.11');
...
SELECT IFNULL(null, 'Hello World'), IFNULL('ㅋㅋㅋㅋㅋㅋ', 'Hello World');

-- IF + ISNULL
SELECT  IF(ISNULL(null),'이름없음','이름있음') AS NAME; -- null 이 있으면 이름있음 없으면 이름 없음

-- IF
-- IF(조건, '조건이 일치 할 경우의 값','조건이 일치하지 않을 경우의 값')
SELECT
        menu_name
       ,IF(orderable_status = 'Y','주문 가능', '주문 불가능') orderable_status
    FROM
        tbl_menu;
/*
선택함수 case
...
-- 해당 컬럼의 값이 null인 행을 제외하고 값을 구한다.
...
-- *는 모든 컬럼(하나의 행)을 의미한다. 행이 존재하면 1로 카운트를 진행한다.
