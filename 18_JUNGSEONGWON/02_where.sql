...
-- tbl_menu 테이블에서 가격이 13000원인 메뉴이름, 메뉴 가격, 주문여부 컬럼을 출력
...
#         orderable_status != 'Y';
#         orderable_status  = 'n';
         orderable_status  = 'N'; -- MYSQL에서는 비교나 검색을 수행할 때 기본적으로 대소문자 구분없이 비교 및 검색이 가능하다.

-- 1. 대소 비교 연산자와 함께 WHERE절 사용
SELECT
         menu_name
        ,menu_price
        ,orderable_status
...
--    0은 FALSE, 0외 숫자는 TRUE로 암시적으로 형변환 후 평가가 된다.
--    문자열은 0으로 반환, FALSE로 평가가 된다.
...
-- 메뉴 테이블에서 주문여부가 Y이면서 카테고리코드가 10인 메뉴목록을 조회
SELECT
          menu_code
         ,menu_price
         ,category_code
...
-- 메뉴 테이블에서 메뉴 가격이 5000원보다 크고, 카테고리 코드가 10인 메뉴를 출력해주세요.
-- 단, 컬럼의 출력은 메뉴 코드, 메뉴 이름, 메뉴 가격, 카테고리 코드, 주문여부만 출력
...
-- 3. OR 연산자와 함께 WHERE 절 사용
SELECT 1 OR 1, 1 OR 0, 0 OR 1;
SELECT 0 OR 0;
SELECT 1 OR NULL, 0 OR NULL, NULL OR NULL;

-- 메뉴 테이블에서 주문여부가 Y 이거나  카테고리코드가 10인 메뉴목록을 조회
SELECT
           menu_code
         , menu_price
         , category_code
...
-- 우선 순위
-- 나열한 AND OR 중에는 AND가 우선 순위가 더 높다.
SELECT  1 OR 0 AND 0;
SELECT  (1 OR 0) AND 0;


-- 카테고리 번호가 4 또는 가격이 9000원이면서 메뉴 번호가 10번보다 큰 메뉴를 조회하자
...
SELECT
    ...
    WHERE
    #       IFNULL(ref_category_code, 0) = 0; -- IFNULL()는 만약 ()안의 내용이 NULL, MYSQL에서만 사용가능
