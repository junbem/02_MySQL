...
-- 문자열 리스트의 ,뒤에 공백(띄어쓰기)가 들어가면 안된다.
SELECT FIND_IN_SET('바나나', '사과,딸기,바나나');


-- ORDER BY절에서 특정 레코드를 끌어올리기 위한 용도로 사용할 수 있다.
SELECT *
  FROM
      tbl_menu
 ORDER BY
     FIND_IN_SET(menu_code, 12) DESC, FIND_IN_SET(menu_code, 15) DESC;
...
SELECT FORMAT(1234351835837548173.12645, 3);
SELECT
    menu_code
  , FORMAT(menu_price, 0) '단위'
  FROM
      tbl_menu;

-- INSERT(기준 문자열, 위치, 길이, 삽입할 문자열)
-- INSERT: 기준 문자열의 위치부터 길이만큼을 지우고 삽입할 문자열을 끼워 넣는다.
SELECT INSERT('내 이름은 지정호 입니다.', 7, 3, '홍길동');
SELECT
...
-- 출력값 1235 , 1234 , -1
SELECT ROUND(1234.56), ROUND(1234.567, 1), TRUNCATE(1234.56, 1), TRUNCATE(-0.65, 0);
...
-- 1부터 10까지 난수(랜덤숫자) 발생 : FLOOR(RAND() * 10 + 1)
...
-- 출력값 2025-03-21 15:15:31,2025-03-22 15:15:31,2025-03-20 15:15:31
SELECT ADDDATE('2025-03-21', INTERVAL 30 DAY), SUBDATE('2025-03-21', INTERVAL 5 MONTH);
-- 출력값 2025-04-20,2024-10-21
...
-- 출력값 2024-03-21 16:19:01,2024-03-21 11:18:59
...
SELECT DATEDIFF('2025-03-21', NOW()), TIMEDIFF('18:00:00', '15:32:12');
...
SELECT EXTRACT(MICROSECOND FROM NOW()); -- now()에는 microsecond가 없다.
SELECT EXTRACT(MICROSECOND FROM '2025-03-21 15:39:28.27398472948374');
...
-- 정수형으로 데이터 형변환
SELECT
    CAST(AVG(menu_price) AS SIGNED integer) '평균 메뉴 가격'
  FROM
      tbl_menu;
-- 출력값 10881
SELECT
    CONVERT(AVG(menu_price), SIGNED integer) '평균 메뉴 가격'
  FROM
      tbl_menu;
-- 출력값 10881
...
SELECT
  ...
  FROM
      tbl_menu;
-- menu_price가 문자로 변환된다.
SELECT 3 > 'MAY'; -- 문자는 0으로 변환된다.
SELECT 5 > '6MAY', '6MAY' + 1;
SELECT ADDDATE('2025-03-11', 1);
...
-- *는 모든 컬럼(하나의 행)을 의미한다. 행이 존재하면 1로 카운트 진행
SELECT
    COUNT(*)
  , COUNT(category_code)
  FROM
...
-- 숫자(크고/작고 기준), 문자열(가나다순), 날짜/시간(빠름/느림 기준)에 대해서도 최대/최소를 처리할 수 있다.
SELECT
    MAX(menu_price)
  , MIN(menu_price)
  , MAX(menu_name)
