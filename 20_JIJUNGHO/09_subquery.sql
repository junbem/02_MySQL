...
SELECT -- 메인쿼리
 ...
 WHERE
     -- 서브쿼리
     category_code = (SELECT category_code FROM tbl_menu WHERE menu_name = '열무김치라떼');
...
SELECT *
  FROM
      tbl_menu
 WHERE
     category_code IN (SELECT category_code FROM tbl_category WHERE ref_category_code = 1);
...
SELECT
 ...
 WHERE
   ...
   AND menu_price > ALL (SELECT menu_price FROM tbl_menu WHERE category_code = 4);
...
SELECT
  ...
  FROM
      tbl_menu a
 WHERE
     menu_price = (SELECT MAX(menu_price) FROM tbl_menu b WHERE b.category_code = a.category_code);

-- 카테코리별 평균 가격보다 높은 가격의 메뉴를 조회
SELECT
    menu_code
  , menu_name
  , menu_price
  ...
  FROM
      tbl_menu a
 WHERE
     menu_price > (SELECT AVG(menu_price) FROM tbl_menu b WHERE b.category_code = a.category_code);
...
SELECT
    a.menu_name
  , (SELECT category_name FROM tbl_category b WHERE b.category_code = a.category_code) AS category_name
  FROM
      tbl_menu a;
