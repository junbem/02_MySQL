...
# -- AND &&    여러 조건이 동시에 TRUE일 경우에만 TRUE 값 반환 (앞에 값이 false 뒤에 값을 안보겠지)
# -- OR  || 여러 조건들 중에 어느 하나의 조건만 TRUE이면 TRUE값 반환 (앞에 값이 true면 뒤에 값을 안보겠지)
...
SELECT 1 AND 1, 2 && 2, -1 AND 1, 1 && 'abc';
