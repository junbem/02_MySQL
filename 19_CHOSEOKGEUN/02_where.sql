...
-- tbl_menu 테이블에서 가격이 13000원인 메뉴이름, 메뉴가격, 주문여부컬럼을 출력
SELECT
       menu_name
     , menu_price
     , orderable_status
...
SELECT
 ...
 WHERE
       orderable_status = 'N';
#      orderable_status = 'n';
-- MySQL은 비교나 검색을 수행할 때 기본적으로 대소문자 구분없이 비교 및 검색이 가능하다.
#      orderable_status != 'Y';
#      orderable_status <> 'Y';
...
-- 0은 FALSE, 0외 숫자는 TRUE로 암시적 형변환 후 평가가 된다.
-- 문자열은 0으로 반환, FALSE로 평가
-- NULL과의 연산결과는 null이다(0 && null제외)

SELECT 1 AND 1, 2 && 2;  -- TRUE
SELECT 1 AND 1, -1 && 1; -- TRUE
SELECT 1 AND 1, -1 && 'abc'; -- FALSE
SELECT 1 AND 0; -- FALSE
SELECT 1 AND 0, 0 AND 1; -- FALSE
SELECT 1 AND 0, 0 AND 1, 0 AND 0; -- FALSE
SELECT 1 AND 0, 0 AND 1, 0 AND 0, 0 AND NULL; -- FALSE
SELECT 1 AND NULL; -- null
SELECT 1 AND NULL, NULL AND NULL; -- null
SELECT 1 + NULL; -- null
SELECT 1 + NULL, 1 - NULL; -- null
SELECT 1 + NULL, 1 - NULL, 1 * NULL; -- null
SELECT 1 + NULL, 1 - NULL, 1 * NULL, 1 / NULL; -- null
...
SELECT
 ...
 WHERE
   ...
   AND
       category_code = 10;
-- 메뉴테이블에서 메뉴가격이  5000원보다 크고, 카테고리 코드가 10인 메뉴를 출력해주세요
...
SELECT 1 OR 1; -- TRUE
SELECT 1 OR 1, 1 OR 0; -- TRUE
SELECT 1 OR 1, 1 OR 0, 0 OR 1; -- TRUE
SELECT 0 OR 0; -- FALSE
SELECT 1 OR NULL; -- TRUE
SELECT 1 OR NULL, 0 OR NULL; -- null
SELECT 1 OR NULL, 0 OR NULL, NULL OR NULL; -- null
...
SELECT 1 OR 0 AND 0; -- TRUE
SELECT (1 OR 0) AND 0; -- FALSE

-- 카테고리번호가 4 또는 가격이 9000원이면서 메뉴번호가 10번보다 큰 메뉴를
-- 모든 컬럼을 조회
SELECT
       menu_code
     , menu_name
...
-- 문자%     : 문자로 시작하는 내용만
-- %문자%    : 문자가 포함되어 있는 내용만
-- SELECT * FROM users WHERE name LIKE 'J%';  -- 'J'로 시작하는 이름 검색
-- SELECT * FROM users WHERE name LIKE '%son';  -- 'son'으로 끝나는 이름 검색
-- SELECT * FROM users WHERE name LIKE '_a%';  -- 두 번째 문자가 'a'인 이름 검색
...
#     IFNULL(ref_category_code, 0) = 0;
      COALESCE(ref_category_code, 0) = 0; -- 모든 BDMS에서  사용가능
