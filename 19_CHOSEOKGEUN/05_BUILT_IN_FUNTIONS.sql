...
-- https://namu.wiki/w/%EC%95%84%EC%8A%A4%ED%82%A4%20%EC%BD%94%EB%93%9C?from=%EC%95%84%EC%8A%A4%ED%82%A4%EC%BD%94%EB%93%9C
-- ASCII(아스키 코드) 중 대소문자 'A~Z' 'a~z' 는 외워 두기
...
-- BIT_LENGTH: 할당된 비트 크기 반환 / 비트는 0 와 1로 구성되어있다
-- CHAR_LENGTH: 문자열의 길이 반환
-- LENGTH : 할당된 BYTE 크기

SELECT
      menu_name, BIT_LENGTH(menu_name)
  FROM
    tbl_menu;

SELECT
    menu_name, BIT_LENGTH(menu_name), CHAR_LENGTH(menu_name)
FROM
    tbl_menu;

SELECT
    menu_name, BIT_LENGTH(menu_name), CHAR_LENGTH(menu_name), LENGTH(menu_name)
FROM
    tbl_menu;
...
-- ORDER BY에서 특정 레코드를 끌어올리기 위한 용도로 사용할 수 있다. (돈 더 주면 혜택주는 광고랑 비슷)
SELECT  * FROM tbl_menu ORDER BY FIND_IN_SET(menu_code, 20) DESC;
...
SELECT FORMAT(1234123412341234.123456,3);
...
-- ROUND: 반올림값 반환
SELECT CEILING(1234056), FLOOR(1234.56), FLOOR(-0.65);
...
-- 년월일은 더하거나 뺄 수 없다.
SELECT ADDTIME('2024-03-21 15:19:00', '3:0:1');
SELECT SUBTIME('2024-03-21 15:19:00', '4:0:1');
...
SELECT DATEDIFF('2025-09-09', NOW()), TIMEDIFF('18:00:00', '15:32:12');

-- EXTRACT(UNIT FORM 날짜시간)
...
SELECT DAYOFWEEK(CURDATE()), MONTH(CURDATE()), DAYOFYEAR(CURDATE());
...
-- 메뉴의 가격을 출하는데 원을 붙여서 출력
SELECT CONCAT(CAST(menu_price AS CHAR(5)), '원')
FROM tbl_menu;

# 암시적  형변환(Implicit Conversion)
SELECT '1' + '2';
SELECT CONCAT(menu_price, '원') FROM tbl_menu; -- menu-price 가 문자로 변환된다.
...
-- IF + ISNULL
SELECT IF(ISNULL('홍길동'), '이름없음', '이름있음') AS NAME;
...
-- *는 모든 컬럼(하나은행)을 의미한다. 행을 존재하면 1로 카운트
SELECT
      COUNT(*) -- NULL 포함 COUNT
    , COUNT(tbl_category.ref_category_code) -- NULL 미포함 COUNT
  FROM
      tbl_category;
