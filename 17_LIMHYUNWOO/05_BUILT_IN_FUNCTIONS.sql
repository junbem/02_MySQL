...
SELECT         -- 찾는 시작점은 다르지만 반환하는 숫자는 같음(단어의 시작점을 반환하기 떄문)
       INSTR('사과딸기바나나', '딸기')
     , LOCATE('딸기', '사과딸기바나나');
...
SELECT FORMAT(123412341321321.5165465,3);
SELECT menu_code,FORMAT(menu_price,0) 단위 FROM tbl_menu ORDER BY menu_price ASC;

-- INSERT(기준 문자열, 위치, 길이, 삽입할 문자열)
-- INSERT: 기준 문자열의 위치부터 길이만큼을 지우고 삽입할 문자열을 끼워 넣는다.
SELECT  INSERT('내 이름은 아무개 입니다.', 7, 3,'임현우');
...
SELECT LEFT('Hellow World', 4), RIGHT('Hellow World', 5);

-- UPPER(문자열), LOWER(문자열)
-- UPPER: 소문자를 대문자로 변경
-- LOWER: 대문자를 소문자로 변경
SELECT Lower('Hellow World'), UPPER('Hello World');
...
SELECT ROUND(1234.56), ROUND(1234.567,2), TRUNCATE(1234.56,1), TRUNCATE(-1.65,0);
...
SELECT EXTRACT(MICROSECOND FROM '2025-03-21 15:12:33.1111');
...
-- IF
-- IF(조건, '조건이 일치할 경우의 값', '조건이 일치하지 않으 경우의 값')
SELECT
       menu_name
