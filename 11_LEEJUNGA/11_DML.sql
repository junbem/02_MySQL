...
-- 문법1. 테이블의 선언될 컬럼 순서 그대로 모두 값을 전달해야한다.
INSERT INTO tbl_menu VALUES (null, '바나나맛해장국', 8500, 6,'Y');
-- 묹법2. NULL 허용 가능한(NULLABLE) 컬럼이나 AUTO_INCREAMENT가 있는 컬럼을 제외하고
...
UPDATE tbl_menu -- 수정할 대상 테이블
SET category_code = 10, -- 여러개의 값을 동시에 수정할 경우 콤마(,)를 사용한다.
    menu_name = '매생이쉐이크'
WHERE menu_code = 28;

-- SUBQUERTY를 활용할 수 있다.
-- 다만 MySQL은 Oracle과 달리 update나 delete시 자기 자신 테이블의 데이터를 사용할 시 1093 에러가 발생한다.
UPDATE tbl_menu
SET category_code = (
        SELECT menu_code
        FROM tbl_menu
